...
export const createWorkspace = async (workspace: Pick<Workspace, 'name' | 'description'>): Promise<Workspace> => {
  try {
...
export const updateWorkspace = async (id: string, workspace: Partial<Pick<Workspace, 'name' | 'description'>>): Promise<Workspace> => {
  try {
    const { data, error } = await supabase
      .from('workspaces')
      .update(workspace)
